...
async function startServer() {
  ...
  app.get("/uploads/:filename", async (req, res) => {
    ...
    res.status(404).send("הקובץ לא נמצא בשרת.");
  });
  ...
  app.get("/api/claims/:id", async (req, res) => {
    const { id } = req.params;
    try {
      if (!ObjectId.isValid(id)) return res.status(400).json({ error: "מזהה לא תקין" });
      const claim = await db.collection("claims").findOne({ _id: new ObjectId(id) });
      if (!claim) return res.status(404).json({ error: "התביעה לא נמצאה" });
  ...
  app.put("/api/claims/:id", async (req, res) => {
    ...
    try {
      if (!ObjectId.isValid(id)) return res.status(400).json({ error: "מזהה תביעה לא תקין" });
      
      let objectId: ObjectId;
      try {
        objectId = new ObjectId(id);
      } catch (e) {
        return res.status(400).json({ error: "מזהה תביעה לא תקין" });
      }

      const currentClaim = await db.collection("claims").findOne({ _id: objectId });
      if (!currentClaim) return res.status(404).json({ error: "התביעה לא נמצאה" });
      ...
      res.json({ success: true, last_activity_at: updateData.last_activity_at });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
